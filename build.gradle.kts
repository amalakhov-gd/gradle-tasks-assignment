...
private fun getSortingType(project: Project): SortingType {
    ...
    return when (sortType) {
        null,
        SORT_TYPE_CREATION_DATE -> SortingType.CREATION_DATE

        SORT_TYPE_EXTENSION -> SortingType.EXTENSION
        else -> {
            throw InvalidUserDataException("Invalid property `${SORT_TYPE_PROPERTY}`. Valid values are: [`${SORT_TYPE_CREATION_DATE}`, `${SortingType.EXTENSION}`]. value provided")
        }
    }
}
